...
def check_addresses():
    ...
    if not addresses:
        ...
        output_text.insert("end", "Vui lòng nhập địa chỉ.")
        result_label.config(text="Kết quả: Vui lòng nhập địa chỉ.")
        return
    ...
    # Cập nhật thông báo thời gian thực thi và số lượng địa chỉ hợp lệ
    elapsed_time = time.time() - start_time
    total_addresses = len(addresses)
    valid_count = len(valid_addresses)
    result_label.config(text=f"Kết quả: Đã kiểm tra xong! {valid_count}/{total_addresses} địa chỉ hợp lệ. Thời gian thực thi: {elapsed_time:.2f} giây")
...
label_input = tk.Label(root, text="OpenStreetMap Nhập địa chỉ (mỗi địa chỉ một dòng):")
